...
.PHONY: setup show all test lcov install check cmakelint format clean distclean run-clang-tidy
...
check: setup .configure-$(BUILD_TYPE) .clang-tidy
...
distclean:
	rm -rf $(BUILD_DIR) .configure-* .*.log compile_commands.json *~ .*~ tags
	find . -name '*~' -delete
